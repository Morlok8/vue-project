...
const fetchData = async () => {
  ...
  try {
    ...
    incomes.value = response.data.data; // Предполагаем, что данные в поле data
    ...
    if(inStock.value){
      ...
      incomes.value = incomes.value.filter((stock) =>  stock.is_supply === true);
    }
  } catch (err) {
