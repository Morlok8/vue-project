...
const fetchData = async () => {
  ...
  try {
    ...
    orders.value = response.data.data; // Предполагаем, что данные в поле data
    ...
    if(inStock.value){
      ...
      orders.value = orders.value.filter((stock) =>  stock.is_supply === true);
    }
  } catch (err) {
